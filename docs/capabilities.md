...
codex  primary_agent — via prompt_append
omp  primary_agent — via prompt_append
claude  prompt_append — via primary_agent
opencode  prompt_append — via primary_agent
omp  model_literal_binding — via model_class_binding
claude  model_class_binding — via model_literal_binding
codex  model_class_binding — via model_literal_binding
...
claude  skip  per_agent_bash_policy  lead
...
claude  skip  external_directory_policy  agent:lead.permissions.external_directory
    agent "lead" sets permissions.external_directory; this harness has no external-directory access policy, so it was dropped.
claude  reduction  compose_into_primary  agent:plan
...
claude  reduction  structured_workflow_command  command:ship
...
codex  skip  agent_steps  agent:build.steps
    agent "build" sets steps: 40; this harness has no step-budget mechanism, so the step limit is dropped.
codex  skip  per_agent_bash_policy  lead
    this harness has no per-agent bash scoping; only the global bash profile is applied, harness-wide, so per-agent profile overrides are dropped.
codex  skip  external_directory_policy  agent:lead.permissions.external_directory
    agent "lead" sets permissions.external_directory; this harness has no external-directory access policy, so it was dropped.
codex  skip  agent_task_permission  agent:lead.permissions.task
...
codex  skip  agent_task_permission  agent:build.permissions.task
...
codex  reduction  structured_workflow_command  command:ship
    command "ship" has 3 steps; this harness has no native structured-workflow mechanism, so it behaves as flattened, numbered prose here instead of a deterministic multi-phase pipeline.

...
- **omp**: a step needing "restricted planning/review discipline for the primary" compiles to content composed into the primary's `APPEND_SYSTEM.md` (per #13's already-prototyped `compose_into_primary` direction) rather than a standalone agent file, since omp has no static per-agent tool-enforcement surface (#9) and reserved-name collisions (#14) make standalone role-named agent files fragile; a step needing "independently dispatchable tool" (research, QA-shaped work) still compiles to a standalone `~/.omp/agent/agents/*.md` file, since that shape genuinely fits omp's `task` tool.
...
| Schema reversibility | ✅ No change needed | ❌ Hard to reverse once real registries exist (this is exactly why an ADR was requested) | ⚠️ Two concepts to maintain long-term is its own lock-in |
| Solves #9 (omp tool enforcement) | ❌ No | ✅ Yes, by routing "restricted discipline" steps to compose-into-primary instead of an unenforceable standalone agent | ⚠️ Only for workflow-opted-in registries |
...
- Positive: gives omp a real (if partial) answer to #9 — supervised-discipline steps get their restriction enforced via composition into the primary's own prompt, instead of an unenforceable standalone file.
...
- Negative: v1's shipped schema, both renderers, `validate`, `doctor`/`capabilities.md`, `docs/schema.md`, and the one example registry all need a breaking rewrite — this is not additive.

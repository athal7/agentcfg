...
- ADR-0001 (workflow as primary registry abstraction, Accepted and implemented, PR #26) — complementary, not conflicting: ADR-0001 relocates the authoring-level unit from standalone agents to workflow steps, but preserves per-harness role compilation for opencode (see its Decision section); this ADR's grant mechanism attaches to whatever registry node compiles to that role — `Agent` today, a workflow step's compiled role under ADR-0001

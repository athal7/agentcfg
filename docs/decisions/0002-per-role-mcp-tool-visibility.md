...
1. **Permission boundary** — a role that omits a server from its `mcp:` list is provably unable to invoke that server's tools (`<server>_*: false` at the harness level, never overridden for that role). Without this, every role — including a narrowly-scoped one with `bash: deny` — would still be able to discover and call any registered server's full tool surface, which is exactly the kind of shell-out-adjacent bypass `docs/schema.md`'s validation warning about MCP-proxy agents already flags for the opposite direction (an agent granted MCP without `bash: deny`).
...
- Positive: default-deny at the harness level means a role that omits a server from its `mcp:` list is provably unable to call that server's tools, independent of what any other role in the same registry is granted.
...
- Negative: omp currently declares neither `CapMCPToolGlobs` nor any per-agent-ask capability (`internal/render/omp/omp.go`'s `Capabilities()`) and exposes every server it targets through one flat `mcpServers` map with no per-role filtering at all. A registry's careful scoped `mcp:` grants silently degrade to "every targeted server visible to every omp agent," and `internal/render/gaps.go`'s `detectMCPToolGlobsGaps` only fires on an `MCPServer.Tools` allowlist — not on a role's `Ask` list — so this asymmetry produces no gap warning today.
...
- `docs/schema.md`: `agents:`/`mcp:` (L240-243, 283-288), `mcp_servers:` (L290-315)
- `examples/registry/agents.yaml`: `build` agent's `mcp:` entry (L30-32)

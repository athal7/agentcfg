...
// renderAgentFiles builds one WriteFile per role: delegate agent targeting
// omp. role: primary is the session itself (no file); role: advisory is
// spliced into the primary's APPEND_SYSTEM.md instead (see
...
func renderAgentFiles(reg *registry.Registry, readFile func(string) ([]byte, error)) ([]render.WriteFile, error) {
	...
	for _, a := range reg.Agents {
		if a.Role != "delegate" || !targets(a.Targets) {
			continue
		}
